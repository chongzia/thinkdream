<template>
  <div class="tickets-container">
    ...
    <!-- 工单详情抽屉 -->
    <el-drawer v-model="dialogState.ticketDetail" title="工单详情" direction="rtl" size="70%"
      :before-close="() => closeDialog('ticketDetail')">
      <TicketDetailViewer v-if="dialogState.ticketDetail && currentData.ticketRow"
        :ticket-id="currentData.ticketRow.id" />
    </el-drawer>
...
interface DialogState { ticketDetail: boolean; formData: boolean }
interface CurrentData { ticketRow: any; formData: string | undefined; templateId: number | undefined; }
interface DataCache { users: Map<number, string>; statusOptions: Map<number, string>; priorityOptions: Map<number, string>; loading: boolean; }
const dialogState = reactive<DialogState>({ ticketDetail: false, formData: false })
const currentData = reactive<CurrentData>({ ticketRow: null, formData: undefined, templateId: undefined })
const dataCache = reactive<DataCache>({ users: new Map(), statusOptions: new Map(), priorityOptions: new Map(), loading: false })
const isInitialized = ref(false)

// ===== 数据服务 =====
class DataService {
  // 防止重复请求的标记
  private static isUsersFetching = false
  private static isDictFetching = false
  ...
  static async fetchUsers(cache: DataCache): Promise<void> {
    if (this.isUsersFetching || cache.loading || cache.users.size > 0) { return }
      ...
      if (response.data?.data) {
        ...
        response.data.data.forEach((user: any) => {
          if (user && typeof user === 'object' && user.id && user.nickname) {
            ...
            if (!isNaN(userId) && userName) { cache.users.set(userId, userName) }
          }
        })
      }
    }
    catch (error) { ElMessage.error('获取用户列表失败') }
    finally { cache.loading = false; this.isUsersFetching = false }
  }
  static async fetchDictionaries(cache: DataCache): Promise<void> {
    if (this.isDictFetching || cache.statusOptions.size > 0 || cache.priorityOptions.size > 0) { return }
    this.isDictFetching = true
    try {
      const [statusResponse, priorityResponse] = await Promise.all([http.get('options/ticketStatus'), http.get('options/ticketPriority')])
      if (statusResponse.data?.data) {
        statusResponse.data.data.forEach((item: any) => {
          if (item && typeof item === 'object' && (typeof item.value === 'number' || typeof item.value === 'string') && item.label) {
            const value = typeof item.value === 'number' ? item.value : parseInt(item.value)
            const label = String(item.label)
            if (!isNaN(value) && label) { cache.statusOptions.set(value, label) }
          }
        })
      }
      if (priorityResponse.data?.data) {
        priorityResponse.data.data.forEach((item: any) => {
          if (item && typeof item === 'object' && (typeof item.value === 'number' || typeof item.value === 'string') && item.label) {
            const value = typeof item.value === 'number' ? item.value : parseInt(item.value)
            const label = String(item.label)
            if (!isNaN(value) && label) { cache.priorityOptions.set(value, label) }
          }
        })
      }
    }
    catch (error) { ElMessage.error('获取字典数据失败') }
    finally { this.isDictFetching = false }
  }
  static async initializeAll(cache: DataCache): Promise<void> {
    await Promise.all([this.fetchUsers(cache), this.fetchDictionaries(cache)])
  }
}
...
class TicketUtils {
  static getUserName(userId: number | string, userMap: Map<number, string>): string {
    ...
    if (typeof userId === 'object' && userId !== null) { userId = String(userId) }
    if (typeof userId === 'string' && userId.includes(',')) { return userId.split(',').map(id => id.trim()).filter(id => id).map(getSingleUserName).join(', ') }
  ...
  static formatTime(timestamp: number): string {
    ...
    try {
      let time: number
      if (typeof timestamp === 'object' && timestamp !== null) { return '-' }
      if (typeof timestamp === 'string') { time = parseInt(timestamp); if (isNaN(time)) return '-' }
      else { time = timestamp }
  ...
  static getCurrentProcessor(row: any, userMap: Map<number, string>): string {
    try {
      ...
      if (!row.ticket_process_at) { return this.getUserName(acceptUserId, userMap) }
        ...
        if (processUserIds) {
          ...
          try {
            if (typeof processUserIds === 'string') {
              ...
              if (parsed && Array.isArray(parsed)) { userIds = parsed }
              else if (parsed && typeof parsed === 'object') { userIds = [String(parsed)] }
              else if (typeof parsed === 'string') { userIds = parsed.split(',').map(id => id.trim()).filter(id => id) }
              else { userIds = processUserIds.split(',').map(id => id.trim()).filter(id => id) }
            }
            else if (Array.isArray(processUserIds)) { userIds = processUserIds.map(id => String(id)) }
            else if (typeof processUserIds === 'object' && processUserIds !== null) { userIds = [String(processUserIds)] }
            else { userIds = [String(processUserIds)] }
          }
          catch (jsonError) { userIds = String(processUserIds).split(',').map(id => id.trim()).filter(id => id) }
  ...
  static calculateAcceptOverdueStatus(row: any): string {
    try {
      ...
      if (typeof row.created_at === 'string') { createdTime = new Date(row.created_at).getTime() }
      else if (typeof row.created_at === 'number') { createdTime = row.created_at > 1000000000000 ? row.created_at : row.created_at * 1000 }
      else { return '正常' }
      const acceptDeadline = createdTime + (row.ticket_accept_days * 24 * 60 * 60 * 1000)
      const currentTime = Date.now()

      const acceptUserId = row.ticket_process_position || row.ticket_node_accept
      if (!acceptUserId) { return currentTime > acceptDeadline ? '超期' : '正常' }
      if (row.ticket_process_at) { return '正常' }
      return currentTime > acceptDeadline ? '超期' : '正常'
    }
    catch (error) { return '正常' }
  }
  ...
  static calculateProcessOverdueStatus(row: any): string {
    try {
      if (row.closed_at) return '正常'
      if (!row.ticket_accept_days || !row.ticket_process_days || row.ticket_accept_days <= 0 || row.ticket_process_days <= 0) { return '正常' }
      let createdTime: number
      if (typeof row.created_at === 'string') { createdTime = new Date(row.created_at).getTime() }
      else if (typeof row.created_at === 'number') { createdTime = row.created_at > 1000000000000 ? row.created_at : row.created_at * 1000 }
      else { return '正常' }
      ...
      if (row.ticket_process_at) {
        ...
        if (typeof row.ticket_process_at === 'number') { processTime = row.ticket_process_at > 1000000000000 ? row.ticket_process_at : row.ticket_process_at * 1000 }
        else { processTime = new Date(row.ticket_process_at).getTime() }
...
onMounted(async () => {
  ...
  await DataService.initializeAll(dataCache)
})
...
const searchForm = computed(() => {
  // 详细检查数据加载状态
  const statusLoaded = dataCache.statusOptions.size > 0
  const priorityLoaded = dataCache.priorityOptions.size > 0
  const usersLoaded = dataCache.users.size > 0
  const hasData = statusLoaded && priorityLoaded && usersLoaded

  const statusOptions = hasData ? Array.from(dataCache.statusOptions.entries()).map(([value, label]) => ({ value, label })) : []
  const priorityOptions = hasData ? Array.from(dataCache.priorityOptions.entries()).map(([value, label]) => ({ value, label })) : []
  const userOptions = hasData ? Array.from(dataCache.users.entries()).map(([value, label]) => ({ value, label })) : []
