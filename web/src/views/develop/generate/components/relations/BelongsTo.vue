<template>
  <el-form ref="formRef" :model="formData" :rules="rules" label-position="top">
    ...
    <el-form-item label="外键" prop="foreignKey">
      ...
      <Prompt text="当前模型中的外键，默认为关联模型名称加上 _id (如 user_id)" />
    </el-form-item>

    <el-form-item label="关联模型主键" prop="ownerKey">
      <el-select v-model="formData.ownerKey" filterable allow-create placeholder="选择或输入主键" class="w-full">
        <el-option v-for="field in availableFields" :key="field" :label="field" :value="field" />
      </el-select>
      <Prompt text="关联模型的主键 (通常是 id)" />
    </el-form-item>

    <el-form-item label="关联方法名" prop="relation">
      <el-input v-model="formData.relation" placeholder="例如: user" />
...
// 使用模型关系 hook
const { modules, availableFields, relatedModelFields, handleRelatedModelChange, getSingularRelationName } = useModelRelation()

const model = defineModel<BelongsToParams>({
  default: () => ({
    relatedModel: '',
    foreignKey: '',
    ownerKey: '',
