...
class Model extends Creator
{
    ...
    public function getContent(): string|bool|PhpFile
    {
        ...
        // 是否关闭页码
        if (! $this->isPaginate) {
            $modelClass->addProperty('isPaginate', $this->isPaginate)->addComment('关闭分页');
        }

        $asTree = in_array('parent_id', $tableColumns) || in_array('pid', $tableColumns);
        // 添加搜索字段
        if ($this->softDelete) {
        ...
        foreach ($this->structures as $structure) {
            ...
            if ($structure['form_component'] == 'upload-image') {
                ...
                if ($structure['list']) {
                    $modelFuncLike = $modelFuncLike->addBody('get: fn ($value) => url($value),');
                }
